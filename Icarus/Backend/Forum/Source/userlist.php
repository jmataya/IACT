		...
		<form id="userlist" method="get" action="userlist.php">
			<div class="inform">
				<!--<fieldset>
					...
					<div class="infldset">
<?php if ($pun_user['g_search_users'] == '1'): ?>						<label class="conl"><?php echo Search ?><br /><input type="text" name="username" value="<?php echo pun_htmlspecialchars($username) ?>" size="25" maxlength="25" /><br /></label>
...
if ($db->num_rows($result))
{
	...
	// Grab the users
	$result = $db->query('SELECT u.id, u.username, u.title, u.num_posts, u.registered, g.g_id, g.g_user_title, u.email, u.realname, i.school, i.department, i.degree, i.picture_url, i.discipline FROM '.$db->prefix.'users AS u LEFT JOIN '.$db->prefix.'groups AS g ON g.g_id=u.group_id LEFT JOIN icarus_user_profile AS i ON u.id = i.user_id WHERE u.id IN('.implode(',', $user_ids).') ORDER BY '.$sort_by.' '.$sort_dir.', u.id ASC') or error('Unable to fetch user list', __FILE__, __LINE__, $db->error());
...
<div id="viewprofile" class="block">
    <img id="profile-pic" src="<?php echo $user_data['picture_url'] ?>"></img>
    <div id="infoBox">
        <h2><span><?php /*echo $lang_common['Profile']*/ echo $user_data['realname'] ?></span></h2>
        <ul>
        <?php
            if ($user_data['discipline'] == 1)
            {
        ?>
                <li><?php echo $user_data['school'] ?></li>
                <li><?php echo $user_data['department'] ?></li>
                <li><?php echo $user_data['degree'] ?></li>
        <?php
            }
        ?>
                <li><a href="mailto:<?php echo $user_data['email'] ?>"><?php echo $user_data['email'] ?></a></li>
        </ul>
    </div>
</div>
</div></div></div></div></div></div>
